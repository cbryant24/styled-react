...
// TODO: ADD ABILITY FOR DELAY_BETWEEN "in" AND "continuous" ANIMATIONS
export const validateAnimation = animation => {
  ...
  if ('in' in animation) {
    ...
    if (!('duration_in' in animation)) {
      throw new TypeError(
        'If you have an in animation you need to specify a duration for that animation see https://github.com/cbryant24/chapivia/'
      );
    } else {
      ...
      if (!checkForValidDuration(animation.duration_in)) {
        throw new TypeError(
          ...
          } is not a valid duration in for an animation should be an integer 0 or greater`
        );
      }
    }
    ...
    if ('delay_between' in animation) {
      throw new TypeError(
        `You cannot have a delay between in and out animations if you're missing any of them see https://github.com/cbryant24/chapivia/`
      );
    }
  }
  ...
  if ('out' in animation) {
    ...
    if (!('duration_out' in animation)) {
      throw new TypeError(
        'If you have an out animation you need to specify a duration for that animation see https://github.com/cbryant24/chapivia/'
      );
    }
    ...
    if (!checkForValidDuration(animation.duration_out)) {
      throw new TypeError(
        ...
        } is not a valid duration out for an animation should be an integer 0 or greater`
      );
    }
  } else {
    if ('delay_between' in animation) {
      throw new TypeError(
        `You cannot have a delay_between animations if not using out animation see https://github.com/cbryant24/chapivia/`
      );
    }
  }
  ...
  if ('in' in animation && 'out' in animation) {
    ...
    if (!('delay_between' in animation)) {
      throw new TypeError(
        'If you have an in animation and an out animation you need to specify a delay between those animations see https://github.com/cbryant24/chapivia/'
      );
    }
    // Check for Valid Animation Duration In
    if (!checkForValidDuration(animation.duration_out)) {
    ...
    if ('duration_continuous' in animation) {
      ...
      if (!checkForValidDuration(animation.duration_continuous)) {
        throw new TypeError(
          ...
          } is not a valid duration for a continuous animation should be an integer 0 or greater`
        );
      }
    } else {
      //TODO: ADD LINK TO README DOCUMENTATION
      throw new TypeError(
        'If you have a continous animation and you must specify a duration_continuous see https://github.com/cbryant24/chapivia/'
      );
    }
  }
};
